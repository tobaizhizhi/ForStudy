...
> 不依赖任何闭源钱包，全部走**开放标准**。
> 最后更新：2026-06。协议状态以当时公开信息为准；x402 / A2A / AP2 / ERC-8004 仍在快速演进，文中所有"现状数字"与"production-ready / 已上线"类断言都应在进入对应 Layer 前现场重新核实（见下方"协议成熟度风险"）。
...
- **演进中（用，但别焊死）**：x402、A2A、AP2 —— 方向明确，但版本、采用度、治理归属仍在变；做可插拔设计。
- **草案 / 早期（最后再碰）**：ERC-8004 —— 目前仍处 ERC 草案 / 早期实现阶段，不要把项目核心架构押在它的稳定性上，放到最后作为可插拔的身份/信誉层。
...
第一轮先**暂缓**复杂 AP2 mandate、ERC-8004 信誉注册、跨链信誉迁移、完整服务市场撮合。等 MVP v1 跑通后，再把 AP2 用作高风险授权层，把 ERC-8004 用作公开身份与信誉层。
...
> MCP 与 A2A 现在都归 **Linux Foundation 的 Agentic AI Foundation** 治理；AP2 也已捐给 Linux Foundation；x402 由 **Coinbase + Cloudflare 共治的 x402 Foundation** 维护。（以上治理归属请核实，可能仍在变化。）这套栈是值得持续跟踪的一组候选协议，但**别把架构焊死在任一标准的当前形态上**——成熟度由高到低大致是：EIP-712/1271、ERC-4337、MCP（基础层，优先投入）> x402 / A2A / AP2（演进中）> ERC-8004（草案/早期）。
...
- Smart Account SDK（选一个上手，优先 SDK，不手写 `UserOperation` / EntryPoint / bundler / paymaster）：
  - [ZeroDev](https://docs.zerodev.app)（session key / policy 体验好）
  - [Safe{Core}](https://docs.safe.global)（多签 + 模块，适合资金池）
  - [Pimlico / permissionless.js](https://docs.pimlico.io)（bundler/paymaster 基础设施）
...
- **A2A v1.0**（2026 初稳定）：
  - **Agent Card**（能力声明）+ **Signed Agent Card**（v1.0 新增，加密签名防伪造，是去中心化发现能信任的前提）
  - 发现（discovery）、任务（task）、消息（message）
  - 多协议绑定（JSON-RPC + gRPC）、多租户、版本协商
  - SDK：Python / JS / Java / Go / .NET
- **MCP vs A2A 边界**：MCP 接工具，A2A 接 agent，两个一起用
- **ERC-8004（Trustless Agents，⚠️ 草案 / 早期实现阶段，进入前先核实标准状态，勿把项目核心架构押在它的稳定性上）** —— agent 的链上身份与信誉：
  - 三个链上注册表：**Identity / Reputation / Validation**
  - 每个 agent = 一个 **ERC-721 NFT**，指向一份 **Agent Card JSON**（含名称、能力、MCP/A2A/web 端点、收款地址）
  - 扩展 A2A，加上"无需预先信任也能选择对方 agent"的信任层
  - 信誉可携带：在以太坊积累的记录能跟着 agent 迁到 Base / Optimism
- **协议适配层设计**：给 A2A、MCP、HTTP、未来协议留统一 adapter 接口；Agent Card / capability / endpoint / auth / rate limit / payment hint 这些字段要能映射到统一内部模型
- **发现与路由**：Agent Card 解析、签名校验、版本协商、endpoint health check、超时与重试、能力匹配、任务路由；不是只“找到 agent”，而是能稳定把任务发给它
- **服务目录**：做一个轻量 registry / directory，列出可用 agent、能力标签、链支持、收费方式、信誉摘要、健康状态；让“发现”有一个可视化入口
- **互操作测试**：同一任务能通过 MCP 调工具、A2A 发任务、HTTP 取结果；测试中故意喂无签名 Card、错版本 Card、过期 Card、伪造 endpoint，看系统是否拒绝

**为什么**
这是"agent-to-agent"真正的骨架：A2A 负责发现和协作，MCP 负责工具接入，ERC-8004 负责身份与信誉。Layer 6 要做成“协议网关”和“服务目录”，让 Layer 5 生成的工作流可以跨 agent、跨工具、跨服务稳定流转。

**资源**
- [modelcontextprotocol.io](https://modelcontextprotocol.io) · [MCP 规范](https://spec.modelcontextprotocol.io)
- [a2a-protocol.org](https://a2a-protocol.org) · [github.com/a2aproject/A2A](https://github.com/a2aproject/A2A)
- ERC-8004 讨论与规范：[Ethereum Magicians: ERC-8004 Trustless Agents](https://ethereum-magicians.org/t/erc-8004-trustless-agents/25098)

**里程碑**：做一个“Agent Protocol Gateway” —— 同时支持 MCP server、A2A agent card discovery 和 HTTP fallback；带一个轻量服务目录，能列出 agent、能力、链支持、收费方式和信誉摘要；两个 agent 可以互相发现并完成一次有签名、有版本协商、有错误回放的任务交接。

**安全验收**：未签名 / 签名不匹配 / 版本不兼容 / endpoint 被篡改的 Agent Card 必须拒绝；服务目录里显示的 endpoint 与实际验证结果要一致；协议适配层不能绕过认证、额度、计费和日志。
...
- **x402**（核心结算层）：
  ...
  - **Facilitator**（结算撮合者）：Coinbase 在 Base/Solana/Stellar 提供免费公共 facilitator；MVP 优先用公共 facilitator，不自建
  - 链无关设计，主力在 **Base / Solana**（低费、快确认）；结算币种以 **USDC** 为主，**EIP-3009** 实现 gasless，任意 ERC-20 经 Permit2 支持
  - 现状（2026，⚠️ 以下数字与"production-ready"说法均需进入本层前现场核实，勿作为架构依据）：据报道已破 1.65 亿+ 笔交易、6.9 万+ 活跃 agent，零协议费；Visa（TAP）、Stripe（ACP）被报道已接入
- **AP2**（授权层，与 x402 互补）：
  - 三种 **Mandate**：Intent（意图）/ Cart（购物车）/ Payment（付款），以 **W3C Verifiable Credentials** 形式签名
  - AP2 本身**不动钱**，只产出"这笔交易已被授权"的可验证记录，由具体 rail（卡/银行/稳定币）结算
  - **A2A x402 extension**：让一个 AP2 mandate 去授权一笔 x402 USDC 结算，给 crypto 支付与刷卡同级的审计链 —— 这是把"授权"和"结算"接起来的关键（仓库 `google-a2a/a2a-x402`；成熟度/版本进入本层前核实）
- **稳定币基础**：USDC / EURC、EIP-3009 gasless transfer 机制
...
- **收费模型**：按次、按流量、按任务、按成功结果；系统里至少要支持一种最小计费单位，并能把价格和结算币种显示给用户
...
**资源**
- x402：[x402.org](https://www.x402.org) · [github.com/coinbase/x402](https://github.com/coinbase/x402)
- AP2：[ap2-protocol.org](https://ap2-protocol.org) · [github.com/google-agentic-commerce/AP2](https://github.com/google-agentic-commerce/AP2) · x402 扩展 [github.com/google-a2a/a2a-x402](https://github.com/google-a2a/a2a-x402)
...
**里程碑**：做一个“Paywalled Agent Service” —— A 请求 B 的服务，先拿报价，再走 AP2/x402 授权与结算，B 返回结果并在前端展示支付状态、交易 hash、回执和对账信息；高风险金额必须进入人工审批，失败支付能重试或标记退款。
...
✅ 产物：用 ZeroDev / Safe / Pimlico / Alchemy Account Kit 等成熟 AA 工具创建一个智能账户 + 一把受限 session key（作用域/额度/过期）+ gasless 领取；不手写 bundler/paymaster/EntryPoint 交互。
...
在 4A 的付费服务外层优先用 A2A SDK 接入 A2A：发布 Agent Card / Signed Agent Card，让另一个 agent 能发现它、读取能力说明，并发起一次任务调用。
✅ 产物：两个 agent 互相发现并完成一次"付费服务调用"，支付仍沿用 4A 的 x402 流程。
🔐 安全验收：校验 Signed Agent Card 的签名、服务端点、版本和能力声明；拒绝未签名/签名不匹配/端点被篡改的 Agent Card，避免调用伪装 agent。
...
支付授权:    AP2 SDK / A2A x402 extension（Intent/Cart/Payment mandate）
支付结算:    x402 SDK + 公共 facilitator + USDC（EIP-3009 gasless）
agent 钱包:  ERC-4337 / 7702 + session key + paymaster
             （ZeroDev / Safe / Pimlico）
...
| A2A | Agent2Agent，agent↔agent 通信/发现 |
| Agent Card | A2A 中描述 agent 能力的清单；Signed 版加密签名防伪 |
| AP2 | Agent Payments Protocol，用签名 mandate 表达支付授权 |
| Mandate | AP2 中的 Intent/Cart/Payment 授权凭证（W3C VC） |
| x402 | 基于 HTTP 402 的 agent 稳定币支付协议 |
| Facilitator | x402 中负责验证与结算的撮合方 |
| ERC-8004 | Trustless Agents：agent 链上身份/信誉/验证标准（草案 / 早期阶段，状态需核实） |
| USDC / EURC | 主流合规稳定币 |
...
- x402 现状与采用：[stablecoininsider.org/x402-protocol](https://stablecoininsider.org/x402-protocol/) · [eco.com x402 解释](https://eco.com/support/en/articles/12328618-x402-protocol-explained-how-ai-agents-pay-onchain)
- A2A v1.0 与 Linux Foundation：[Linux Foundation: A2A 一周年](https://www.linuxfoundation.org/press/a2a-protocol-surpasses-150-organizations-lands-in-major-cloud-platforms-and-sees-enterprise-production-use-in-first-year) · [github.com/a2aproject/A2A](https://github.com/a2aproject/A2A)
- ERC-8004 状态（草案 / 早期实现；"主网上线 / 数千身份"等说法以一手规范与官方仓库为准，二手媒体谨慎对待）：[Ethereum Magicians 讨论帖（一手）](https://ethereum-magicians.org/t/erc-8004-trustless-agents/25098) · [news.bitcoin.com ERC-8004（二手，谨慎）](https://news.bitcoin.com/what-is-erc-8004-ethereums-new-agent-standard-powers-thousands-of-onchain-ai-identities/)
- AP2 与 x402 扩展：[Google Cloud: 宣布 AP2](https://cloud.google.com/blog/products/ai-machine-learning/announcing-agents-to-payments-ap2-protocol) · [Crossmint: 支付协议对比](https://www.crossmint.com/learn/agentic-payments-protocols-compared)
