...
其中前四条“作用域 / 额度 / 过期 / 撤销”，在 `layer4/contracts` 的 23 个 Foundry 测试里已经逐条覆盖——这是链上强制的证据，不是口头承诺。

## 9.6 最小测试策略

| 层级 | 覆盖对象 | 工具 |
| --- | --- | --- |
| 合约单元/集成 | `validateUserOp` 各策略维度、EIP-1271、撤销、过期、日额度 | Foundry（已 23 个测试） |
...
7. 为什么说 Foundry 的 23 个测试是“权限边界的证据”？
8. 里程碑控制台至少要能完成哪几个动作？
