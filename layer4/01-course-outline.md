...
```text
layer4/
  00-先跑起来：Layer4学习导读.md  推荐入口：先跑本地 lab，再跑真实链脚本
  ...
  agent-wallet-lab/               本地可运行练习（不需要私钥 / 不上链）
    src/01-user-operation.ts       拆 UserOperation / initCode / 外层 txHash
    src/02-validation-data.ts      拆 validationData 的签名状态与时间窗
    src/03-session-policy.ts       本地模拟 session key 策略闸门
    src/04-paymaster-simulation.ts 模拟 bundler 链下模拟 + paymaster 赞助决策
    src/05-eip1271.ts              本地演示智能账户如何验 TaskIntent
    src/06-eip7702.ts              演示 7702 里 from / to / 委托代码的关系
